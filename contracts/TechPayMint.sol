...
import "./modules/TechPayMintBalanceGuard.sol";
import "./modules/TechPayMintCollateral.sol";
import "./modules/TechPayMintDebt.sol";
import "./modules/TechPayMintConfig.sol";

// TechPayMint implements the contract of core DeFi function
// for minting tokens against a deposited collateral. The collateral
// management is linked from the TechPay Collateral implementation.
// Minting is burdened with a minting fee defined as the amount
// of percent of the minted tokens value in tUSD. Burning is free
// of any fee.
contract TechPayMint is Initializable, TechPayMintBalanceGuard, TechPayMintCollateral, TechPayMintDebt, TechPayMintConfig {
    // define used libs
    ...
    function initialize(address owner, address _addressProvider) public initializer {
        ...
        addressProvider = ITechPayMintAddressProvider(_addressProvider);

        // init the underlying party
        TechPayMintConfig.initialize(owner);
        TechPayMintCollateral.initialize();
        TechPayMintDebt.initialize();
    }
    ...
    modifier onlyLiquidationManager() {
        require(msg.sender == address(addressProvider.getTechPayLiquidationManager()), "token storage access restricted"); 
        _;       
    }
