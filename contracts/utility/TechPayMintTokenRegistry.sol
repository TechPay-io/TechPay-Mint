...
* author TechPay
*/
contract TechPayMintTokenRegistry is Initializable, Ownable, ITechPayMintTokenRegistry {
    ...
    // canMint informs if the specified token can be minted in TechPay DeFi.
    function canMint(address _token) public view returns (bool) {
        return tokens[_token].canMint;
    }

    // canTrade informs if the specified token is tradable in TechPay DeFi.
