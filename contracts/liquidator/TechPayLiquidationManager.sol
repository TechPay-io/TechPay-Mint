...
import '../TechPayMint.sol';
import '../utility/TechPayMintTokenRegistry.sol';
import '../modules/TechPayMintBalanceGuard.sol';

// TechPayLiquidationManager implements the liquidation model
// with the ability to fine tune settings by the contract owner.
contract TechPayLiquidationManager is
  Initializable,
  Ownable,
  TechPayMintErrorCodes,
  ReentrancyGuard
{
  // define used libs
  ...
  function updateTechPayMintContractAddress(address _techpayMintContract)
    external
    onlyOwner
  {
    techpayMintContract = _techpayMintContract;
  }
  ...
  function collateralIsEligible(address _account) public view returns (bool) {
    return
      TechPayMint(addressProvider.getAddress(MOD_TECHPAY_MINT))
    ...
    nonReentrant
  {
    ...
    ITechPayDeFiTokenStorage collateralPool = getCollateralPool();
    ...
    for (index = 0; index < _auction.collateralList.length; index++) {
      ...
      TechPayMint(techpayMintContract).settleLiquidationBid(
    ...
    onlyNotContract
  {
    ...
    ITechPayDeFiTokenStorage collateralPool = getCollateralPool();
    // get the debt pool
    ITechPayDeFiTokenStorage debtPool = getDebtPool();
    ...
    for (index = 0; index < tokenCount; index++) {
      ...
      if(TechPayMintTokenRegistry(addressProvider.getAddress(MOD_TOKEN_REGISTRY)).canTrade(tokenAddress)){
